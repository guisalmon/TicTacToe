...
public class Player {
	...
	/**
	 * Says whether the player has already moved in this game or not
	 * 
	 * @return true if the player has already moved
	 */
	...
	/**
	 * Says whether the player owns or not the button b given in parameter
	 * 
	 * @param b
	 * @return true if the player owns the button
	 */
	...
	/**
	 * Indicates whether a move allows victory or not regarding the previous
	 * positions owned by the player
	 * 
	 * @param b
	 *            New move
	 * @return True if the move allows victory
	 */
	...
	/**
	 * Indicates whether a move creates two victory opportunities or not
	 * regarding the previous positions owned by the player and the free
	 * positions on the board
	 * 
	 * @param newMove
	 *            New move
	 * @param slots
	 *            List of the buttons of the board
	 * @return True if the move creates two victory opportunities
	 */
	public boolean isFork(Button newMove, List<Button> slots) {
		...
		for (Button b : slots) {
			p1 = new Position(b);
			if (b.isClickable() && hypPlayer.isVictory(b)
					&& ((p.x != p1.x) || (p1.y != p.y))) {
				branch++;
				break;
			}
		...
		for (Button b : slots) {
			...
			if (b.isClickable() && hypPlayer.isVictory(b) && p1 != null
					&& ((p2.x != p1.x) || (p1.y != p2.y))
					&& ((p.x != p2.x) || (p2.y != p.y))) {
				branch++;
				break;
	...
	/**
	 * Indicates whether a move creates a victory opportunity or not regarding
	 * the previous positions owned by the player and the free positions on the
	 * board
	 * 
	 * @param newMove
	 *            New move
